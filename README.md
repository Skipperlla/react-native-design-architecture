...
- `src`: This folder is for the source code of the app.
  ...
  - `constants`: This folder is designated for the constants of the application. The boilerplate uses this folder for constants that are used throughout the application. If a constant is used in more than one file, you should create a file for that constant in this folder. Constants should be named in **macro case** format (**ALL_UPPERCASE** with words separated by underscores). The constants are exported in an object format. Additionally, all constants must be exported from the `src/constant/index.ts` file. You can access the constants from the `@app/constants` directory.
